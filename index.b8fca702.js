    ...
    [
        ...
        [
            ...
            if (currentValue !== 0) {
                ...
                if (prevValue === 0) cell.classList.add("field-cell--new");
                else if (prevValue !== currentValue) cell.classList.add("field-cell--merged");
            }
        });
    });
    gameScore.textContent = game.getScore();
    previousState = fields.map((row)=>[
            ...row
        ]);
}
...
class Game {
   ...
   */ constructor(initialState = [
        ...
        if (Array.isArray(initialState) && initialState.length === MAX_FIELDS && initialState.every((e)=>e.length === MAX_FIELDS)) {
    ...
    moveLeft() {
        if (this.getStatus() !== "lose") {
            ...
            if (previousState !== JSON.stringify(this.initialState)) this.#crateField();
    ...
    static #shiftLeft(matrix) {
        ...
        for(let i = 0; i < result.length; i++){
            ...
            for(let m = 0, n = m + 1; n < row.length; n++, m++)if (row[m] === row[n]) {
                row[m] *= 2;
                score += row[m];
                row[n] = 0;
            }
            row = row.filter((e)=>e !== 0);
            while(result[i].length !== row.length)row.push(0);
            result[i] = row;
        }
        return {
            result,
    ...
    static #transposition(result) {
        for(let i = 0; i < result.length; i++)for(let j = i + 1; j < result[i].length; j++)[result[i][j], result[j][i]] = [
            result[j][i],
            result[i][j]
        ];
        return result;
   ...
   */ start() {
    ...
    #crateField() {
        ...
        for(let i = 0; i < this.initialState.length; i++){
            for(let j = 0; j < this.initialState[i].length; j++)if (this.initialState[i][j] === 0) emptyCells.push({
                x: i,
                y: j
            });
        }
   ...
   */ restart() {
    ...
    #isLose() {
        for(let i = 0; i < this.initialState.length; i++)for(let j = 0; j < this.initialState[i].length; j++){
            if (this.initialState[i][j] === 0) return false;
            if (j < this.initialState[i].length - 1 && this.initialState[i][j] === this.initialState[i][j + 1]) return false;
            if (i < this.initialState.length - 1 && this.initialState[i][j] === this.initialState[i + 1][j]) return false;
        }
        return true;
    }
}
